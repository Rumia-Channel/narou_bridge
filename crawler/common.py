...
def _load_json_safe(
    path: str, create_backup: bool = True, max_backups: int = 3
) -> Dict:
    """
    JSON読み込みヘルパー（安全版）
    ...
    try:
        ...
        logging.error(
            f"JSONDecodeError: {path} is corrupted at line {e.lineno}: {e.msg}"
        )
...
def check_image_file(
    img_path: str, file_name: str, is_cover_check: bool = False
) -> Optional[str]:
    """
    ...
    if _global_img_url:
        base_url = (
            _global_img_url if _global_img_url.endswith("/") else _global_img_url + "/"
        )
    else:
    ...
    for key, value in database.items():
        if is_cover_check:
            ...
            key_match = key == file_name or key.split(".")[0] == base_name
            value_match = (
                value == base_name or value + os.path.splitext(key)[1] == file_name
            )
...
def check_image_hash(
    img_path: str, file_data: bytes, file_name: str, is_cover: bool = False
) -> str:
    """
    ...
    # ハッシュ計算
    image_hash = (
        base64.urlsafe_b64encode(hashlib.sha3_256(file_data).digest())
        .rstrip(b"=")
        .decode("utf-8")
    )
...
def save_raw_diff(raw_path: str, novel_path: str, novel: dict):
    ...
    if diff_json:
        # ファイル名に使えない文字を置換
        date_str = (
            str(old_json.get("get_date", "unknown")).replace(":", "-").replace(" ", "_")
        )
...
def gen_site_index(folder_path: str, key_data, site_name: str):
    ...
    if no_raw:
        logging.warning(
            f"The folders {', '.join(no_raw)} were deleted because they do not contain 'raw.json'."
        )
