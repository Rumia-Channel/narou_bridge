...
def extract_text_with_details(pdf_path: str) -> Tuple[List[Dict], List[Dict]]:
    ...
    for i in tqdm.tqdm(
        range(1, total_pages - 1), leave=False, desc="Loading", unit="characters"
    ):
...
def extract_second_highest_group_line(last_details: List[Dict]) -> Optional[str]:
    ...
    if len(sorted_y_keys) > 1:
        second_highest_y = sorted_y_keys[
            0
        ]  # 元コードのロジックでは sorted_y_keys[0] を取得して除外せずそのまま使っている箇所があるが、
...
def sort_text_details(text_details: List[Dict]) -> List[Dict]:
    ...
    return sorted(text_details, key=lambda item: (item["page"], -item["x"], item["y"]))


def _identify_structure(
    data: List[Dict], max_x_page_2: float, min_y_coordinate: float
...
def _reconstruct_section_text(
    data: List[Dict], start_page: int, end_page: int, max_x_ignore: float
) -> str:
    """
    ...
    # 行間の推定
    sizes = sorted(
        {item["size"] for item in target_items if "size" in item}, reverse=True
    )
    main_font_size = sizes[0] if sizes else 0
    line_space = (
        (sizes[0] + sizes[1] + 1) if len(sizes) >= 2 else LINE_SPACE_THRESHOLD_DEFAULT
    )

    # メインサイズのみ抽出して座標計算の基準にする
    main_text_items = [
        item for item in target_items if item.get("size") == main_font_size
    ]
...
def process_text_details(
    ...
    titles, introductions, postscripts = _identify_structure(
        file_data, max_x_page_2, min_y_coordinate
    )
    ...
    for key in sorted(titles.keys()):
        if key in introductions:
            ordered_sections.append(
                {"key": key, "type": "intro", "page": introductions[key]["page"]}
            )
        ordered_sections.append(
            {"key": key, "type": "main", "page": titles[key]["page"]}
        )
        if key in postscripts:
            ordered_sections.append(
                {"key": key, "type": "post", "page": postscripts[key]["page"]}
            )
    ...
    for i in tqdm.tqdm(
        range(len(ordered_sections)), desc="Reconstructing", unit="sec", leave=False
    ):
        start_p = boundaries[i]
        end_p = boundaries[i + 1]

        # テキスト復元
        text_segment = _reconstruct_section_text(
            file_data, start_p, end_p, max_x_page_2
        )
    ...
    # 5. メタデータ抽出
    novel_title, n_code, author, introduction_text = _parse_introduction_metadata(
        full_texts[0]
    )
    ...
    results = {
        ...
        "type": "novel",
        "serialization": {0: "連載中", 1: "完結済", 2: "短編"}.get(
            novel_type, "連載中"
        ),
        "createDate": gen_date,
    ...
    for key in sorted(titles.keys()):
        ep_id = key - 1
        if key == 1:
            continue  # 1話目はメタデータ部分に含まれることが多いためスキップ(元コード準拠)
...
def gen_from_pdf(
    ...
    # 2. 構造化データへの変換
    results, ncode = process_text_details(
        sorted_details, last_details, author_id, author_url, novel_type, chapter
    )
...
def repair(folder_path: str, key_data: str, data_path: str, host_name: str):
    ...
    logging.info("=" * 60)
    logging.info("Data Repair (narou): Starting full data repair...")
    logging.info("=" * 60)
    ...
    for name in folder_names:
        work_path = os.path.join(folder_path, name)
    ...
    for name in folder_names:
        ...
        try:
            ...
            logging.error(f"Rebuild failed for {name}: {e}")
    ...
    logging.info("")
    logging.info("=" * 60)
    logging.info(f"Data Repair (narou) Summary:")
